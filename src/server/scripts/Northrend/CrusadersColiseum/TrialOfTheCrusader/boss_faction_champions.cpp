...
struct boss_faction_championsAI : public ScriptedAI
{
    ...
    {
        bool IsTriggeredSpell = bool(pSpell->AttributesEx4 & SPELL_ATTR4_TRIGGERED);
        DiminishingGroup m_diminishGroup = GetDiminishingReturnsGroupForSpell(pSpell,IsTriggeredSpell);
        if (m_diminishGroup)
        {
    ...
    void UpdatePower()
    {
        if (me->getPowerType() == POWER_MANA)
            me->ModifyPower(POWER_MANA, uint32(me->GetMaxPower(POWER_MANA) / 100 * 20));
    ...
    Creature* SelectRandomFriendlyCC(float distance)
    {
        std::list<Creature *> lst = DoFindFriendlyCC(distance);
        std::list<Creature *>::const_iterator itr = lst.begin();
        if (lst.empty())
            return NULL;
        advance(itr, rand()%lst.size());
    ...
    bool DoHealingSpellIfReady(uint32 spell)
    {
        ...
        if (me->isAttackReady())
        {
            if (Unit *pHealTarget = DoSelectLowestHpFriendly(90.0f))
            {
                if (me->IsWithinCombatRange(me->getVictim(), spellInfo->GetMaxRange(false)))
                {
                    me->CastSpell(pHealTarget, spell, false);
                    me->resetAttackTimer();
                }
                else
                    return false;
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (mAIType != AI_PET)
        {
            if (CCTimer < uiDiff)
            {
                if (me->HasAuraType(SPELL_AURA_MOD_STUN) || me->HasAuraType(SPELL_AURA_MOD_FEAR) || me->HasAuraType(SPELL_AURA_MOD_ROOT) || me->HasAuraType(SPELL_AURA_MOD_PACIFY) || me->HasAuraType(SPELL_AURA_MOD_CONFUSE)) 
                {
        ...
        if (mAIType == AI_RANGED) 
            DoSpellAttackIfReady(m_uiAttackSpell);

        if (mAIType == AI_HEALER) 
            DoHealingSpellIfReady(m_uiAttackSpell);
    }
};
...
public:
    ...
    struct mob_toc_druidAI : public boss_faction_championsAI
    {
        ...
        void Reset()
        {
            ...
            m_uiAttackSpell = SPELL_REGROWTH;
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            if (m_uiCommonTimer <= uiDiff)
            {
                ...
                {
                    case 0:
                        if (Unit *healtarget = DoSelectLowestHpFriendly(40.0f))
                        {
                            DoCast(healtarget, SPELL_LIFEBLOOM);
                    ...
                    case 1:
                        if (Unit *healtarget = DoSelectLowestHpFriendly(40.0f))
                        {
                            DoCast(healtarget, SPELL_NOURISH);
                            m_uiCommonTimer = urand(5*IN_MILLISECONDS, 10*IN_MILLISECONDS);
                        }
                        break;
                    case 2:
                        if (Unit *healtarget = DoSelectLowestHpFriendly(40.0f))
                        {
                            DoCast(healtarget, SPELL_REJUVENATION);
                            m_uiCommonTimer = urand(5*IN_MILLISECONDS, 10*IN_MILLISECONDS);
                        }
                        break;
                    case 3:
...
public:
    ...
    struct mob_toc_shamanAI : public boss_faction_championsAI
    {
        ...
        void Reset()
        {
            ...
            m_uiAttackSpell = SPELL_HEALING_WAVE;
            ...
            if (m_uiCommonTimer <= uiDiff)
            {
                switch (urand(0, 3))
                {
                    case 0:
                        if (Unit *healtarget = DoSelectLowestHpFriendly(40.0f))
                        {
                            DoCast(healtarget, SPELL_RIPTIDE);
                            m_uiCommonTimer = urand(5*IN_MILLISECONDS, 10*IN_MILLISECONDS);
                        }
                        break;
                    case 1:
                        ...
                        break;
                    case 2:
                        if (Unit *target = SelectRandomFriendlyCC(40.0f))
                        {
...
public:
    ...
    struct mob_toc_paladinAI : public boss_faction_championsAI
    {
        ...
        void Reset()
        {
            ...
            m_uiCommonTimer = urand(5*IN_MILLISECONDS, 10*IN_MILLISECONDS);
            m_uiAttackSpell = SPELL_FLASH_OF_LIGHT;
            ...
            if (m_uiCommonTimer <= uiDiff)
            {
                switch (urand(0, 2))
                {
                    case 0: case 1:
                        ...
                        break;
                    case 2:
                        if (Unit *pTarget = SelectRandomFriendlyCC(40.0f))
                        {
...
public:
    ...
    struct mob_toc_priestAI : public boss_faction_championsAI
    {
        ...
        void Reset()
        {
            ...
            m_uiAttackSpell = SPELL_FLASH_HEAL;
            ...
            if (m_uiCommonTimer <= uiDiff)
            {
                switch (urand(0, 3))
                {
                    ...
                    case 1:
                        if (Unit *healtarget = DoSelectLowestHpFriendly(40.0f))
                        {
                            DoCast(healtarget, SPELL_SHIELD);
                            m_uiCommonTimer = urand(5*IN_MILLISECONDS, 10*IN_MILLISECONDS);
                        }
                        break;
                    case 2:
...
public:
    ...
    struct mob_toc_shadow_priestAI : public boss_faction_championsAI
    {
        ...
        void Reset()
        {
            ...
            m_uiCommonTimer = urand(15*IN_MILLISECONDS, 30*IN_MILLISECONDS);
            m_uiAttackSpell = SPELL_MIND_FLAY;
...
public:
    ...
    struct mob_toc_warlockAI : public boss_faction_championsAI
    {
        ...
        void Reset()
        {
            ...
            m_uiCommonTimer = urand(15*IN_MILLISECONDS, 30*IN_MILLISECONDS);
            m_uiAttackSpell = SPELL_SHADOW_BOLT;

            SetEquipmentSlots(false, 49992, EQUIP_NO_CHANGE, EQUIP_NO_CHANGE);

            m_uiSummonPetTimer = urand(15*IN_MILLISECONDS, 30*IN_MILLISECONDS);
            DoCastAOE(SPELL_SUMMON_FELHUNTER,true);
        }
...
public:
    ...
    struct mob_toc_mageAI : public boss_faction_championsAI
    {
        ...
        void Reset()
        {
            ...
            m_uiCommonTimer = urand(15*IN_MILLISECONDS, 30*IN_MILLISECONDS);
            m_uiAttackSpell = SPELL_FROSTBOLT;
...
public:
    ...
    struct mob_toc_hunterAI : public boss_faction_championsAI
    {
        ...
        void Reset()
        {
            ...
            m_uiCommonTimer = urand(15*IN_MILLISECONDS, 30*IN_MILLISECONDS);
            m_uiAttackSpell = SPELL_SHOOT;

            SetEquipmentSlots(false, 47156, EQUIP_NO_CHANGE, 48711);

            m_uiSummonPetTimer = urand(15*IN_MILLISECONDS, 30*IN_MILLISECONDS);
            DoCastAOE(SPELL_CALL_PET,true);
        }
