...
class instance_trial_of_the_crusader : public InstanceMapScript
{
    public:
        ...
        struct instance_trial_of_the_crusader_InstanceMapScript : public InstanceScript
        {
            ...
            void ClearBossCorpse(uint32 type)
            {
                switch(type)
                {
                    case TYPE_JARAXXUS:
                        ...
                        switch (data)
                        {
                            case DONE:
                            {
                                if (instance->GetPlayers().getFirst()->getSource()->GetTeam() == ALLIANCE)
                                    ...
                                    EventStage = 4030;
                                DoUpdateAchievementCriteria(ACHIEVEMENT_CRITERIA_TYPE_KILL_CREATURE, NPC_DARKBANE, 1);
                                break;
                            }
                        }
                        break;
                            ...
                            case SPECIAL:
                                ...
                                if (tributeChest)
                                    if (Creature* tirion =  instance->GetCreature(TirionGUID))
                                        if (GameObject* chest = tirion->SummonGameObject(tributeChest, EndingSpawnLoc[1].GetPositionX(), EndingSpawnLoc[1].GetPositionY(), EndingSpawnLoc[1].GetPositionZ(), EndingSpawnLoc[1].GetOrientation(), 0, 0, 0, 0, 7*DAY))
