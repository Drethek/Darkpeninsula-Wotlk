...
void AggroAllPlayers(Creature* pTemp)
{
    if (!pTemp)
        return;
    if (pTemp->isInCombat() || !pTemp->isAlive())
        return;
    ...
    for (Map::PlayerList::const_iterator i = PlList.begin(); i != PlList.end(); ++i)
    {
        if (Player* player = i->getSource())
        {
            ...
            if (player->isAlive())
            {
                ...
                if (Unit *pVehicle = player->GetVehicleBase())
                {
                    pTemp->SetInCombatWith(pVehicle);
                    pTemp->AddThreat(pVehicle, 0.0f);
                }         
...
void GrandChampionsNotInCombat(Creature* me)
{
    ...
    Creature* pGrandChampion1 = Unit::GetCreature(*me, pInstance->GetData64(DATA_GRAND_CHAMPION_VEHICLE_1));
    if (me != pGrandChampion1)
        AggroAllPlayers(pGrandChampion1);
    Creature* pGrandChampion2 = Unit::GetCreature(*me, pInstance->GetData64(DATA_GRAND_CHAMPION_VEHICLE_2));
    if (me != pGrandChampion2)
        AggroAllPlayers(pGrandChampion2);
    Creature* pGrandChampion3 = Unit::GetCreature(*me, pInstance->GetData64(DATA_GRAND_CHAMPION_VEHICLE_3));
    if (me != pGrandChampion3)
        AggroAllPlayers(pGrandChampion3);
}

void EnterInVehicle(Creature *me)
{
    InstanceScript* pInstance = me->GetInstanceScript();
    if (pInstance->GetData64(DATA_GRAND_CHAMPION_1) == me->GetGUID()) {
        Position pos;
        me->GetPosition(&pos);
        uint64 uiVehicleGUID;

        Creature* pBoss = me->SummonCreature(pInstance->GetData64(DATA_GRAND_CHAMPION_VEHICLE_ENTRY_1),pos);

        uiVehicleGUID = pBoss->GetGUID();
        uint64 uiGrandChampionBoss = 0;
        if (Vehicle* pVehicle = pBoss->GetVehicleKit())
            if (Unit* pUnit = pVehicle->GetPassenger(0))
                uiGrandChampionBoss = pUnit->GetGUID();

        if (pInstance)
        {
            pInstance->SetData64(DATA_GRAND_CHAMPION_VEHICLE_1,uiVehicleGUID);
            pInstance->SetData64(DATA_GRAND_CHAMPION_1,uiGrandChampionBoss);
        }
        
        pBoss->SetHomePosition(720.719f, 591.141f, 411.737f, 4.65f);
        AggroAllPlayers(pBoss);
        me->DisappearAndDie();
        return;
    }
    if (pInstance->GetData64(DATA_GRAND_CHAMPION_2) == me->GetGUID()) {
        Position pos;
        me->GetPosition(&pos);
        uint64 uiVehicleGUID;

        Creature* pBoss = me->SummonCreature(pInstance->GetData64(DATA_GRAND_CHAMPION_VEHICLE_ENTRY_2),pos);
        ...
        if (pInstance)
        {
            pInstance->SetData64(DATA_GRAND_CHAMPION_VEHICLE_2,uiVehicleGUID);
            pInstance->SetData64(DATA_GRAND_CHAMPION_2,uiGrandChampionBoss);
        }

        pBoss->SetHomePosition(746.887f, 583.425f, 411.668f, 4.65f);
        AggroAllPlayers(pBoss);
        me->DisappearAndDie();
        return;
    }
    if (pInstance->GetData64(DATA_GRAND_CHAMPION_3) == me->GetGUID()) {
        Position pos;
        me->GetPosition(&pos);
        uint64 uiVehicleGUID;

        Creature* pBoss = me->SummonCreature(pInstance->GetData64(DATA_GRAND_CHAMPION_VEHICLE_ENTRY_3),pos);
        ...
        if (pInstance)
        {
            pInstance->SetData64(DATA_GRAND_CHAMPION_VEHICLE_3,uiVehicleGUID);
            pInstance->SetData64(DATA_GRAND_CHAMPION_3,uiGrandChampionBoss);
        }

        pBoss->SetHomePosition(772.48f, 592.99f, 411.68f, 4.65f);
        AggroAllPlayers(pBoss);
        me->DisappearAndDie();
        return;
    }
...
bool GrandChampionsOutVehicle(Creature* me)
{
    ...
    if (pGrandChampion1 && pGrandChampion2 && pGrandChampion3)
    {
		...
		{
            pGrandChampion1->ExitVehicle();
            pGrandChampion2->ExitVehicle();
            pGrandChampion3->ExitVehicle();
            return true;
        }
    }
...
public:
    ...
    struct generic_vehicleAI_toc5AI : public npc_escortAI
    {
        ...
        void SetData(uint32 uiType, uint32 /*uiData*/)
        {
            switch (uiType)
            {
                case 1:
                    ...
                    AddWaypoint(1, 771.43f, 642.60f, 411.90f);
                    AddWaypoint(2, 779.80f, 617.53f, 411.71f);
                    AddWaypoint(3, 771.09f, 594.63f, 411.62f);
                    AddWaypoint(4, 746.88f, 583.42f, 411.66f);
                    AddWaypoint(5, 715.17f, 583.78f, 412.39f);
                    AddWaypoint(6, 720.71f, 591.14f, 411.73f);
                    uiWaypointPath = 1;
                    break;
                case 2:
                    AddWaypoint(0, 746.45f, 647.03f, 411.57f);
                    ...
                    AddWaypoint(3, 771.09f, 594.63f, 411.62f);
                    AddWaypoint(4, 746.88f, 583.42f, 411.66f);
                    AddWaypoint(5, 746.16f, 571.67f, 412.38f);
                    AddWaypoint(6, 746.88f, 583.42f, 411.66f);
                    uiWaypointPath = 2;
                    break;
                case 3:
                    AddWaypoint(0, 746.45f, 647.03f, 411.57f);
                    ...
                    AddWaypoint(2, 779.80f, 617.53f, 411.71f);
                    AddWaypoint(3, 771.09f, 594.63f, 411.62f);
                    AddWaypoint(4, 777.75f, 584.57f, 412.39f);
        ...
        {
            if (me->HasAura(SPELL_SHIELD) && (spell->Id == 65147 || spell->Id == 63003))
                me->RemoveAuraFromStack(SPELL_SHIELD);
                    ...
                    if (me->GetMap()->IsDungeon() && !players.isEmpty())
                    {
                        for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                        {
                            ...
                            if (player && !player->isGameMaster() && me->IsInRange(player, 10.0f, 30.0f, false))
                            {
                                ...
                                if (player->HasAura(SPELL_SHIELD))
                                    player->RemoveAuraFromStack(SPELL_SHIELD);
                                break;
                            }
                        }
                    }
...
public:
    ...
    struct boss_warrior_toc5AI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            if (me->GetReactState() == REACT_PASSIVE && !me->HasUnitMovementFlag(MOVEMENTFLAG_ONTRANSPORT) && !bHome && !uiPhase && !me->HasAura(SPELL_TRAMPLED))
            {
                if (Unit * pUnit = me->SelectNearestTargetInAttackDistance())
                {   
                    if (pUnit->isCharmedOwnedByPlayerOrPlayer()) 
                    {
                        me->GetMotionMaster()->MoveIdle();
                        if (!me->HasUnitState(UNIT_STAT_CASTING))
                            DoCast(me,SPELL_TRAMPLED);
                    }
                }

                if (!me->HasAura(SPELL_TRAMPLED) && !me->HasUnitState(UNIT_STAT_CASTING))
                   if (Creature * pMount = me->FindNearestCreature((pInstance->GetData(DATA_TEAM) == HORDE ? VEHICLE_ARGENT_WARHORSE : VEHICLE_ARGENT_BATTLEWORG), 100.0f))                                          
                        if (me->GetDistance2d(pMount->GetPositionX(),pMount->GetPositionY()) < 5.0f)                               
                            EnterInVehicle(me);
                        else if (me->GetMotionMaster()->GetCurrentMovementGeneratorType() == IDLE_MOTION_TYPE) 
                        {
                            me->SetStandState(UNIT_STAND_STATE_STAND);
                            me->GetMotionMaster()->MovePoint(0,pMount->GetPositionX(),pMount->GetPositionY(),pMount->GetPositionZ());
                        }
            }       
