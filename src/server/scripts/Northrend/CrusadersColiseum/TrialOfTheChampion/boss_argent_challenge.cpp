        ...
        void DamageTaken(Unit* /*done_by*/, uint32 &damage)
        {
            if (damage >= me->GetHealth())
            {
                ...
                if (!me->HasFlag(UNIT_FIELD_FLAGS,UNIT_FLAG_NON_ATTACKABLE)) 
                {
                    ...
                    if (IsHeroic())
                    {
                        AchievementEntry const *AchievConf = GetAchievementStore()->LookupEntry(ACHIEV_CONF);
                        if (AchievConf)
                        {
                            Map* pMap = me->GetMap();
                            if (pMap && pMap->IsDungeon())
                            {
                                ...
                                for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                                    itr->getSource()->CompletedAchievement(AchievConf);
                            }
                        }
                    }
                }
            ...
            switch (me->GetEntry())
            {
                case NPC_ARGENT_LIGHWIELDER:
                    switch (uiType)
                    {
                        ...
                        case 2:
                            ...
                            AddWaypoint(1, 781.62f, 629.38f, 411.89f);
                            break;
                    }
                    break;
                case NPC_ARGENT_MONK:
                    switch (uiType)
                    {
                        ...
                        case 2:
                            AddWaypoint(0, 756.14f, 655.42f, 411.88f);
                            AddWaypoint(1, 781.35f, 633.14f, 411.90f);
                            break;
                    }
                    break;
                case NPC_PRIESTESS:
                    switch (uiType)
                    {
                        ...
                        case 2:
                            AddWaypoint(0, 756.14f, 655.42f, 411.88f);
                            AddWaypoint(1, 780.43f, 636.68f, 411.91f);
                            break;
                    }
                    break;
            }
