...
public:
    ...
    struct npc_announcer_toc5AI : public ScriptedAI
    {
        ...
        void DoSummonGrandChampion(uint32 uiBoss)
        {
            ...
            {
                case 0:
                    VEHICLE_TO_SUMMON1 = pInstance->GetData(DATA_TEAM) == ALLIANCE ? VEHICLE_MOKRA_SKILLCRUSHER_MOUNT : VEHICLE_MARSHAL_JACOB_ALERIUS_MOUNT;                                        
                    VEHICLE_TO_SUMMON2 = pInstance->GetData(DATA_TEAM) == ALLIANCE ? VEHICLE_ORGRIMMAR_WOLF : VEHICLE_STORMWIND_STEED;                                        
                    break;
                case 1:
                    VEHICLE_TO_SUMMON1 = pInstance->GetData(DATA_TEAM) == ALLIANCE ? VEHICLE_ERESSEA_DAWNSINGER_MOUNT : VEHICLE_AMBROSE_BOLTSPARK_MOUNT;                                        					
                    VEHICLE_TO_SUMMON2 = pInstance->GetData(DATA_TEAM) == ALLIANCE ? VEHICLE_SILVERMOON_HAWKSTRIDER : VEHICLE_GNOMEREGAN_MECHANOSTRIDER;                                        
                    break;
                case 2:
                    VEHICLE_TO_SUMMON1 = pInstance->GetData(DATA_TEAM) == ALLIANCE ? VEHICLE_RUNOK_WILDMANE_MOUNT : VEHICLE_COLOSOS_MOUNT;                                        
                    VEHICLE_TO_SUMMON2 = pInstance->GetData(DATA_TEAM) == ALLIANCE ? VEHICLE_THUNDER_BLUFF_KODO : VEHICLE_EXODAR_ELEKK;                                        
                    break;
                case 3:
                    VEHICLE_TO_SUMMON1 = pInstance->GetData(DATA_TEAM) == ALLIANCE ? VEHICLE_ZUL_TORE_MOUNT : VEHICLE_EVENSONG_MOUNT;                                        
                    VEHICLE_TO_SUMMON2 = pInstance->GetData(DATA_TEAM) == ALLIANCE ? VEHICLE_DARKSPEAR_RAPTOR : VEHICLE_DARNASSIA_NIGHTSABER;                                        
                    break;
                case 4:
                    VEHICLE_TO_SUMMON1 = pInstance->GetData(DATA_TEAM) == ALLIANCE ? VEHICLE_DEATHSTALKER_VESCERI_MOUNT : VEHICLE_LANA_STOUTHAMMER_MOUNT;                                        
                    VEHICLE_TO_SUMMON2 = pInstance->GetData(DATA_TEAM) == ALLIANCE ? VEHICLE_FORSAKE_WARHORSE : VEHICLE_IRONFORGE_RAM;                                        
                    break;
                default:
                    return;
            }
            ...
            if (Creature* pBoss = me->SummonCreature(VEHICLE_TO_SUMMON1, SpawnPosition))
            {
                switch (uiSummonTimes)
                {
                    case 1:
                    {
                        ...
                        if (pInstance)
                        {
                            pInstance->SetData64(DATA_GRAND_CHAMPION_VEHICLE_ENTRY_1, VEHICLE_TO_SUMMON1);
                    ...
                    case 2:
                    {
                        ...
                        if (pInstance)
                        {
                            pInstance->SetData64(DATA_GRAND_CHAMPION_VEHICLE_ENTRY_2, VEHICLE_TO_SUMMON1);
                    ...
                    case 3:
                    {
                        ...
                        if (pInstance)
                        {
                            pInstance->SetData64(DATA_GRAND_CHAMPION_VEHICLE_ENTRY_3, VEHICLE_TO_SUMMON1);
        ...
        void AggroAllPlayers(Creature* pTemp)
        {
            ...
            for (Map::PlayerList::const_iterator i = PlList.begin(); i != PlList.end(); ++i)
            {
                if (Player* player = i->getSource())
                {
                    ...
                    if (player->isAlive())
                    {
                        pTemp->SetHomePosition(pTemp->GetPositionX(), pTemp->GetPositionY(), pTemp->GetPositionZ(), pTemp->GetOrientation());
