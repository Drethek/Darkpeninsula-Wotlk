...
public:
    ...
    struct boss_lich_king_horAI : public npc_escortAI
    {
        ...
        void JustSummoned(Creature* summoned)
        {
            ...
            if (Unit *pLider = me->SelectNearestTarget(999))
            {
                if (pLider->GetTypeId() == TYPEID_PLAYER && pLider->isAlive())
                {
        ...
        void CallGuard(uint32 GuardID, uint8 i)
        {
            if(Creature* Summon = me->SummonCreature(GuardID,(me->GetPositionX()-5)+rand()%10, (me->GetPositionY()-5)+rand()%10, me->GetPositionZ(),4.17f,TEMPSUMMON_TIMED_OR_DEAD_DESPAWN,360000))
        ...
        void Wall01()
        {
            switch(Step)
            {
                ...
                case 1:
                    ...
                    break;
                case 2:
                    m_pInstance->SetData(DATA_RAGING_GNOUL, 0);
                ...
                case 4:
                    ...
                    break;
                case 5:
                    CallGuard(NPC_RISEN_WITCH_DOCTOR, ++count);
        ...
        void UpdateEscortAI(const uint32 diff)
        {
            ...
            if (Creature* pLider = ((Creature*)Unit::GetUnit((*me), m_pInstance->GetData64(DATA_ESCAPE_LIDER))))
            {
                if (pLider->IsWithinDistInMap(me, 2.0f))
                {
                    ...
                    pLider->ForcedDespawn();
                    m_pInstance->SetData(TYPE_LICH_KING, FAIL);
                    m_pInstance->SetData(DATA_SYLVANA_OUTRO, 0);
                    m_pInstance->SetData(DATA_SYLVANA_OUTRO, 1);
                    for(uint8 i = 0; i < 18; i++)
                        if(Creature* Sum = m_pInstance->instance->GetCreature(m_uiSummonGUID[i]))
                            Sum->DespawnOrUnsummon();
                    m_pInstance->SetData(DATA_RAGING_GNOUL, 1);
                    me->ForcedDespawn();
                }
            }

            if(m_pInstance->GetData(TYPE_LICH_KING) == IN_PROGRESS && WinterCast && !me->HasAura(SPELL_WINTER))
                DoCast(me, SPELL_WINTER);
...
public:
    ...
    struct npc_raging_gnoulAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if(LeapTimer < diff)
            {
                me->CastSpell(me->getVictim(), SPELL_GNOUL_JUMP, true);
                LeapTimer = 6000;
            }
            else
                LeapTimer -= diff;
...
public:
    ...
    struct npc_risen_witch_doctorAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            {
                DoCast(me->getVictim(), DUNGEON_MODE(SPELL_COURSE_OF_DOOM, SPELL_COURSE_OF_DOOM_H));
                CurseOfDoomTimer = 15000;
            }
            else
                CurseOfDoomTimer -= diff;

            if(ShadowBoltTimer < diff)
            {
                DoCast(me->getVictim(), DUNGEON_MODE(SPELL_SHADOW_BOLT, SPELL_SHADOW_BOLT_H));
            ...
            {
                DoCast(me->getVictim(), DUNGEON_MODE(SPELL_SHADOW_BOLT_VALLEY, SPELL_SHADOW_BOLT_VALLEY_H));
...
public:
    ...
    struct npc_abonAI : public ScriptedAI
    {
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if(CleaveTimer < diff)
            {
                me->CastSpell(me->getVictim(), SPELL_ABON_STRIKE, true);
                CleaveTimer = 4000;
            }
            else
                CleaveTimer -= diff;
