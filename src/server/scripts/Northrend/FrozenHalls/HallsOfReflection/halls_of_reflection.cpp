...
enum Achievements
{
    ACHIEV_THE_HOR_NORMAL              = 4518,
    ACHIEV_THE_HOR_HEROIC              = 4521,
    ACHIEV_ESCAPE_LICH_KING            = 4526,
};
...
public:
            ...
            case GOSSIP_ACTION_INFO_DEF+1:
                if(m_pInstance)
                {
                    ...
                    for (Map::PlayerList::const_iterator i = PlayerList.begin(); i != PlayerList.end(); ++i)
                    {
                        ...
                        {
                            if(i->getSource()->GetQuestStatus(QUEST_WRATH_OF_THE_LICH_KING_ALLIANCE) == QUEST_STATUS_INCOMPLETE)
                                i->getSource()->KilledMonsterCredit(36955, 0);
                        }
                        else
                        {
                            if(i->getSource()->GetQuestStatus(QUEST_WRATH_OF_THE_LICH_KING_HORDE) == QUEST_STATUS_INCOMPLETE)
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        {
            case NPC_JAINA:
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Lady Jaina, we are ready for next mission!", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
                break;
            case NPC_SYLVANA:
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Lady Sylvanas, we are ready for next mission!", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
                break;
        }
...
public:
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 uiSender, uint32 uiAction)
    {
        ...
        switch (uiAction)
        {
            case GOSSIP_ACTION_INFO_DEF+1:
                player->CLOSE_GOSSIP_MENU();
                m_pInstance->SetData(TYPE_ACHIEV, IN_PROGRESS);
                ...
                if(m_pInstance)
                {
                    // Reset Event if Escape Event Fail
                    if(m_pInstance->GetData(TYPE_LICH_KING) == FAIL)
                    {
                        uint64 m_uiLichKingGUID = m_pInstance->GetData64(BOSS_LICH_KING);
                        if(Creature *pLichKing = m_pInstance->instance->GetCreature(m_uiLichKingGUID))
                            pLichKing->AI()->DoAction(1);
                    }
    ...
    bool OnGossipHello(Player* player, Creature* creature)
    {
        ...
        player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "Let's go!", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1);
    ...
    struct npc_jaina_and_sylvana_HRextroAI : public npc_escortAI
    {
        ...
        bool PreFight;
        bool WallCast;
        bool StartAchievementTimer;
        bool StopAchievementTimer;
        ...
        void Reset()
        {
            ...
            {
                // Cleanup Ice Wall
                m_pInstance->SetData(TYPE_ICE_WALL_01, NOT_STARTED);
                m_pInstance->SetData(TYPE_ICE_WALL_02, NOT_STARTED);
                m_pInstance->SetData(TYPE_ICE_WALL_03, NOT_STARTED);
                m_pInstance->SetData(TYPE_ICE_WALL_04, NOT_STARTED);
                if(GameObject* pGate01 = m_pInstance->instance->GetGameObject(m_pInstance->GetData64(GO_ICE_WALL_1)))
                    pGate01->SetGoState(GO_STATE_ACTIVE);
                if(GameObject* pGate02 = m_pInstance->instance->GetGameObject(m_pInstance->GetData64(GO_ICE_WALL_2)))
                    pGate02->SetGoState(GO_STATE_ACTIVE);
                if(GameObject* pGate03 = m_pInstance->instance->GetGameObject(m_pInstance->GetData64(GO_ICE_WALL_3)))
                    pGate03->SetGoState(GO_STATE_ACTIVE);
                if(GameObject* pGate04 = m_pInstance->instance->GetGameObject(m_pInstance->GetData64(GO_ICE_WALL_4)))
                    pGate04->SetGoState(GO_STATE_ACTIVE);
                // Set Jaina or Sylvana Flag
                me->SetFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_QUESTGIVER);
                me->SetFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP);
                // Reset Var for Restart Escape Event
                Event = true;
                Count = 1;
                WallCast = false;
                HoldTimer = 0;
                ...
                StartAchievementTimer = false;
                StopAchievementTimer = false;
                // Add Jaina or Sylvana Combat State for Correct Escape Event
                me->SetInCombatWithZone();
                me->SetInCombatWith(pLichKing);
            }
 
            if(m_pInstance->GetData(TYPE_LICH_KING) == IN_PROGRESS || m_pInstance->GetData(TYPE_LICH_KING) == FAIL)
                return;
        ...
        void JustDied(Unit* killer)
        {
            if(!m_pInstance)
                return;
            m_pInstance->SetData(TYPE_ACHIEV, FAIL);
            ...
            switch(Step)
            {
                ...
                case 7:
                    ...
                    if(me->GetEntry() == NPC_JAINA_OUTRO)
                    {
    					...
    					me->RemoveAurasDueToSpell(SPELL_ICE_PRISON_VISUAL);
        ...
        void Outro()
        {
            switch(Step)
            {
                ...
                case 11:
                    ...
                    break;
                case 12:
                    if(!StopAchievementTimer)
                        m_pInstance->SetData(TYPE_ACHIEV, DONE);
        ...
        void QuestCompleate()
        {
            if(m_pInstance)
            {
                ...
                {
                    if( (i->getSource()->GetQuestStatus(QUEST_WRATH_OF_THE_LICH_KING_ALLIANCE) == QUEST_STATUS_INCOMPLETE) ||
                        (i->getSource()->GetQuestStatus(QUEST_WRATH_OF_THE_LICH_KING_HORDE) == QUEST_STATUS_INCOMPLETE) )
                        i->getSource()->KilledMonsterCredit(38211, 0);
                }
            }
        }

        void AssignAchievements()
        {		
            if(m_pInstance)
            {
                if ( (m_pInstance->GetData(TYPE_MARWYN) == DONE) && (m_pInstance->GetData(TYPE_FALRIC) == DONE) )
                {
                    if(IsHeroic())
                    {
                        if(m_pInstance->GetData(TYPE_ACHIEV) == DONE)
                            DoCompleteAchievement(ACHIEV_ESCAPE_LICH_KING);
                        DoCompleteAchievement(ACHIEV_THE_HOR_HEROIC);
                    }
                    else
                        DoCompleteAchievement(ACHIEV_THE_HOR_NORMAL);
                }
            }
        }
        ...
        void UpdateEscortAI(const uint32 diff)
        {
            ...
            if(m_pInstance->GetData(TYPE_LICH_KING) == SPECIAL
                && m_pInstance->GetData(TYPE_PHASE) == 5)
            {
            ...
            if(m_pInstance->GetData(TYPE_ACHIEV) == IN_PROGRESS && !StopAchievementTimer)
            {
                if(AchievementTimer < diff)
                {
                    if(!StartAchievementTimer)
                    {
                        AchievementTimer = 360000; // 6 minutes
                        StartAchievementTimer = true;
                    }
                    else
                    {
                        StopAchievementTimer = true;
                        if(m_pInstance->GetData(TYPE_LICH_KING) == DONE)
                            m_pInstance->SetData(TYPE_ACHIEV, DONE);
                        else
    					    m_pInstance->SetData(TYPE_ACHIEV, FAIL);
...
enum GENERAL_EVENT
{
    ...
    SPELL_BALEFUL_STRIKE_H                        = 70400,
    SPELL_SPIRIT_BURST                            = 69900,
    SPELL_SPIRIT_BURST_H                          = 73046,
...
public:
    ...
    struct npc_frostworn_generalAI : public ScriptedAI
    {
        ...
        void SummonSpiritualReflection()
        {
            ...
            for (Map::PlayerList::const_iterator i = players.begin(); i != players.end(); ++i)
            {
                ...
                if (!plr || !plr->isAlive())
                    continue;
                // Summon clone
                if (Unit *summon = me->SummonCreature(NPC_SPIRITUAL_REFLECTION, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), me->GetOrientation(),TEMPSUMMON_CORPSE_DESPAWN,0))
                {				
                    // clone
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            if(m_uiShieldTimer < uiDiff)
            {
                if(Unit *target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                   DoCast(target,DUNGEON_MODE(SPELL_SHIELD_THROWN, SPELL_SHIELD_THROWN_H));
            ...
            if (!isSummon && HealthBelowPct(50))
            {
                SummonSpiritualReflection();
                isSummon = true;
            }
...
public:
    ...
    struct npc_spiritual_reflectionAI : public ScriptedAI
    {
        ...
        InstanceScript* m_pInstance;
        uint32 m_uiBalefulStrikeTimer;
        uint32 m_uiSpiritBurstTimer;

        void Reset()
        {
            m_uiBalefulStrikeTimer = 0;
            m_uiSpiritBurstTimer = 0;
        }
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if(m_uiBalefulStrikeTimer < diff)
            {
                DoCast(me->getVictim(), DUNGEON_MODE(SPELL_BALEFUL_STRIKE, SPELL_BALEFUL_STRIKE_H));
                m_uiBalefulStrikeTimer = urand(8000, 12000);
            }
            else
                m_uiBalefulStrikeTimer -= diff;

            if(m_uiSpiritBurstTimer < diff)
            {
                DoCast(me->getVictim(), DUNGEON_MODE(SPELL_SPIRIT_BURST, SPELL_SPIRIT_BURST_H));
                m_uiSpiritBurstTimer = urand(15000, 20000);
            }
            else
                m_uiSpiritBurstTimer -= diff;
