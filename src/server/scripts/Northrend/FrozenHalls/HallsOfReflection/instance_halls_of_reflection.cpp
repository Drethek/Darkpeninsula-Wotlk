...
Position const JainaSpawnPos = {5577.187f,2236.003f,733.012f,0.0f};
Position const LichKingSpawnPos = {5551.29f,2261.33f,733.012f,4.0452f};
        ...
        void OnCreatureCreate(Creature* creature)
        {
            ...
            switch(creature->GetEntry())
            {
                ...
                case NPC_DARK_RANGER_LORALEN:
                    if (_teamInInstance == ALLIANCE)
                        creature->UpdateEntry(NPC_ARCHMAGE_KORELN, ALLIANCE);
                case NPC_FALRIC: 
                ...
                case NPC_FROST_GENERAL:
                    ...
                    break;
                case NPC_RAGING_GNOUL:
                    ++m_uiRaiseCount;
                    m_uiSummonRaiseGUID[m_uiRaiseCount] = creature->GetGUID();
                    break;
            }
        }
        ...
        void SetData(uint32 uiType, uint32 uiData)
        {
            switch(uiType)
            {
                ...
                case DATA_SUMMONS:
                    ...
                    uiData = NOT_STARTED;
                    break;
                case DATA_RAGING_GNOUL:
                    if(uiData == 0)
                    {
                        m_uiRaiseCount = 0;
                        for (uint8 i = 0; i < 8; ++i)
                            m_uiSummonRaiseGUID[i] = 0;
                    }
                    if(uiData == 1)
                    {
                        m_uiRaiseCount = 0;
                        for (uint8 i = 0; i < 8; ++i)
                            if(Creature* Sum = instance->GetCreature(m_uiSummonRaiseGUID[i]))
                                Sum->DespawnOrUnsummon();
                    }
                    break;
                case DATA_SYLVANA_OUTRO:
                    if(uiData == 0)
                        instance->SummonCreature(NPC_SYLVANA_OUTRO,JainaSpawnPos);
                    if(uiData == 1)
                        instance->SummonCreature(BOSS_LICH_KING,LichKingSpawnPos);
                     break;
            }
