        ...
        struct boss_flame_leviathanAI : public BossAI
        {
            ...
            void InitializeAI()
            {
                ...
                if (me->isAlive())
                    ...
                    {
                        if (Creature *pColossus = DoSummon(NPC_ULDUAR_COLOSSUS, PosColossus[i], 7000, TEMPSUMMON_CORPSE_TIMED_DESPAWN))
                            ColossusGUID[i] = pColossus->GetGUID(); 
                    }
            ...
            void DoAction(int32 const action)
            {
                ...
                switch (action)
                {
                    ...
                    case ACTION_MOVE_TO_CENTER_POSITION: // Triggered by 2 Collossus near door
                        ...
                        break;
                    case ACTION_COLOSSUS_COUNT: // Colossus Count
                        ++ColossusCount;
                        if (ColossusCount >= 2)
                        {
                            Creature* pColossusLeft = me->GetCreature(*me, ColossusGUID[0]);
                            Creature* pColossusRight = me->GetCreature(*me, ColossusGUID[1]);

                            if( (pColossusLeft && pColossusLeft->isDead()) && (pColossusRight && pColossusRight->isDead()) )
                            {
                                // Event starts
                                if (instance)
    ...
    public:
        ...
        struct npc_colossusAI : public ScriptedAI
        {
            ...
            void JustDied(Unit* /*Who*/)
            {
                ...
                if (me->isSummon())
                    if (Creature* leviathan = me->GetCreature(*me, instance->GetData64(BOSS_LEVIATHAN)))
                        leviathan->AI()->DoAction(ACTION_COLOSSUS_COUNT);
            }
...
class npc_lorekeeper : public CreatureScript
{
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature)
        {
            ...
            if ((instance && instance->GetData(BOSS_LEVIATHAN) !=DONE && player &&
                instance->GetBossState(BOSS_LEVIATHAN) != SPECIAL) || player->isGameMaster())
            {
                player->PrepareGossipMenu(creature);
...
#define GOSSIP_ITEM_BRANN_4         "Respawn Vehicle"
...
public:
    ...
    {
        npc_brann_bronzebeardAI(Creature* creature) : npc_escortAI(creature), summons(me)
        {
            instance = creature->GetInstanceScript();
        }
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if(bSteppingBrann)
            {
                if (uiPhaseTimerBrann <= diff)
                {
                    ...
                    {
                        case 0:
                            if (Creature* Lorekeeper = me->FindNearestCreature(NPC_LOREKEEPER, 120, true))
                                Lorekeeper->RemoveFlag(UNIT_NPC_FLAGS, UNIT_NPC_FLAG_GOSSIP);
                        ...
                        case 8:
                            ...
                            if (Creature* leviathan = instance->instance->GetCreature(instance->GetData64(BOSS_LEVIATHAN)))
                            {
                                leviathan->AI()->DoAction(1);
                                leviathan->AI()->DoAction(2);
                                leviathan->AI()->DoAction(3);
                                leviathan->AI()->DoAction(4);
                            }
                            // Spawn Vehicle
                            summons.DespawnAll();
                            for (int32 i = 0; i < RAID_MODE(2, 5); ++i)
    ...
    {
        InstanceScript* instance = creature->GetInstanceScript();
        if ((player && instance && instance->GetBossState(BOSS_LEVIATHAN) != DONE && 
            instance->GetBossState(BOSS_LEVIATHAN) != SPECIAL) || player->isGameMaster())
        {
            if (CAST_AI(npc_brann_bronzebeard::npc_brann_bronzebeardAI, creature->AI())->RespawnVehicle)
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT,GOSSIP_ITEM_BRANN_4,GOSSIP_SENDER_MAIN,GOSSIP_ACTION_INFO_DEF+1);
            else
                player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT,GOSSIP_ITEM_BRANN_3,GOSSIP_SENDER_MAIN,GOSSIP_ACTION_INFO_DEF);
    ...
    bool OnGossipSelect(Player* player, Creature* creature, uint32 uiSender, uint32 uiAction)
    {
        switch(uiAction)
        {
            case GOSSIP_ACTION_INFO_DEF:
                // Start Event Normal Mode
                CAST_AI(npc_brann_bronzebeard::npc_brann_bronzebeardAI, creature->AI())->uiPhaseTimerBrann = 0;
                CAST_AI(npc_brann_bronzebeard::npc_brann_bronzebeardAI, creature->AI())->bSteppingBrann = true;
                CAST_AI(npc_brann_bronzebeard::npc_brann_bronzebeardAI, creature->AI())->uiStepBrann = 0;
