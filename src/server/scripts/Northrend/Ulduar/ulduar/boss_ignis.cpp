        ...
        struct boss_ignis_AI : public BossAI
        {
            ...
            void EnterCombat(Unit* /*who*/)
            {
                ...
                if (me->isAlive())
                    for (uint32 i = 0; i < 20; ++i)
                        DoSummon(NPC_IRON_CONSTRUCT, ConstructSpawnPosition[i], 30000, TEMPSUMMON_CORPSE_DESPAWN);
            }
            ...
            void JustSummoned(Creature* summon)
            {
                if (summon->GetEntry() == NPC_IRON_CONSTRUCT)
                {
                    ...
                    summon->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_PACIFIED | UNIT_FLAG_STUNNED | UNIT_FLAG_DISABLE_MOVE);
            ...
            bool EventSummons()
            {
                ...
                for (std::list<uint64>::const_iterator itr = SummonList.begin(); itr != SummonList.end(); ++itr)
                {
                    if(i == SelectSummons)
                    {
                        if (Creature* pTemp = Unit::GetCreature(*me, *itr))
                        {
                            if (pTemp && !pTemp->isInCombat())
                            {
                                pTemp->setFaction(16);
                                pTemp->SetReactState(REACT_AGGRESSIVE);
                                pTemp->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_PACIFIED | UNIT_FLAG_STUNNED | UNIT_FLAG_DISABLE_MOVE);
