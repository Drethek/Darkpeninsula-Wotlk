    ...
    struct boss_thorimAI : public BossAI
    {
        ...
        void JustDied(Unit * /*victim*/)
        {
            ...
            if (instance)
            {
                ...
                if (HardMode)
                {
                    AchievementEntry const *AchievLoseIllusion = GetAchievementStore()->LookupEntry(ACHIEVEMENT_LOSE_ILLUSION);
                    if (AchievLoseIllusion)
                    {
                        Map* pMap = me->GetMap();
                        if (pMap && pMap->IsRaid())
                        {
                            ...
                            for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                                itr->getSource()->CompletedAchievement(AchievLoseIllusion);
                        }
                    }
        ...
        {
            if (phase == PHASE_1 && killer && instance)
            {
                            ...
                            if (pRuneGiant->isDead())
                                if (me->IsWithinDistInMap(killer, 10.0f) && killer->ToPlayer())
                                {
                                    ...
                                    if (EncounterTime <= MAX_HARD_MODE_TIME)
                                    {
                                        ...
                                        if (instance)
                                        {
                                            AchievementEntry const *AchievSiffed = GetAchievementStore()->LookupEntry(ACHIEVEMENT_SIFFED);
                                            if (AchievSiffed)
                                            {
                                                Map* pMap = me->GetMap();
                                                if (pMap && pMap->IsRaid())
                                                {
                                                    Map::PlayerList const &players = pMap->GetPlayers();
                                                    for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                                                        itr->getSource()->CompletedAchievement(AchievSiffed);
                                                }
                                            }
                                        }
                                    }
