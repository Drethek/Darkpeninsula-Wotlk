    ...
    struct boss_mimironAI : public BossAI
    {
        ...
        void JustDied(Unit *victim)
        {
            ...
            if (instance)
            {
                if (MimironHardMode)
                {
                    AchievementEntry const *AchievFirefighter = GetAchievementStore()->LookupEntry(ACHIEVEMENT_FIREFIGHTER);
                    if (AchievFirefighter)
                    {
                        Map* pMap = me->GetMap();
                        if (pMap && pMap->IsRaid())
                        {
                            ...
                            for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                                itr->getSource()->CompletedAchievement(AchievFirefighter);
                        }
                    }
...
public:
    ...
    struct npc_assault_botAI : public ScriptedAI
    {
        ...
        void SpellHit(Unit *caster, const SpellEntry *spell)
        {
            ...
            {
                AchievementEntry const *AchievNotSoFriendlyFire = GetAchievementStore()->LookupEntry(ACHIEVEMENT_NOT_SO_FRIENDLY_FIRE);
                if (AchievNotSoFriendlyFire)
                {
                    Map* pMap = me->GetMap();
                    if (pMap && pMap->IsRaid())
                    {
                        Map::PlayerList const &players = pMap->GetPlayers();
                        for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                            itr->getSource()->CompletedAchievement(AchievNotSoFriendlyFire);
                    }
                }
            }
        }
...
public:
    ...
    bool OnGossipHello(Player* player, GameObject* pGo)
    {
        ...
        if ((pInstance->GetBossState(BOSS_MIMIRON) != IN_PROGRESS || pInstance->GetBossState(BOSS_MIMIRON) != DONE) && player)
            if (Creature *pMimiron = player->GetCreature((*player), pInstance->GetData64(DATA_MIMIRON)))
