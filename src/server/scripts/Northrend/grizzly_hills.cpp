...
public:
    ...
    struct npc_lake_frogAI : public FollowerAI // FollowerAI: Allows the NPC to follow a target
    {
        ...
        {
            if(following) // If the frog has already received a /kiss it nothing happens
                return;

            if(emote==TEXT_EMOTE_KISS) // If we /kiss
            {
                ...
                {
                    player->RemoveAura(SPELL_WARTSBGONE_LIP_BALM); // It removes the buff set by the quest item
                    me->AddAura(SPELL_FROG_LOVE,me); // Is added will be a frog (hearts)
                    StartFollow(player, 35, NULL); // The frog'm the player
                    following=true;
                }
            }
        }
