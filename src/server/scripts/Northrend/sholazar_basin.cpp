...
#define GOSSIP_ITEM_1 	"I need to find Goregek, do you have his shackles?"

enum eElderHarkek
{
    ITEM_ZEPIK_HUNTING_HORN		= 38512,
...
public:
    ...
    bool IsEligible(Player* pPlayer)
    {
        if ((pPlayer->isInCombat()) || (!pPlayer->IsActiveQuest(QUEST_A_ROUGH_RIDE)) || (pPlayer->HasItemCount(ITEM_ZEPIK_HUNTING_HORN, 1, true)))
            return false;
    }

    bool OnGossipHello(Player* pPlayer, Creature* pCreature)
    {
        if (IsEligible(pPlayer))
    ...
    bool OnGossipSelect(Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
    {
        ...
        {
            if (IsEligible(pPlayer))
                pCreature->CastSpell(pPlayer, SPELL_FORCEITEM_ZEPIK, false);
