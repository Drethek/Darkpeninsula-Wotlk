...
enum RisenAllySpells
{
    SPELL_RAISE_ALLY_TRIGGERED               = 46619,
    SPELL_RAISE_ALLY_GLYPH                   = 60200,
};
...
public:
    ...
    struct npc_risen_allyAI : public ScriptedAI
    {
        ...
        void Reset()
        {
            ...
            {
                float ghoul_speed = me->GetSpeedRate(MOVE_RUN);
                uint32 ghoul_health = uint32(creator->GetMaxHealth() * 0.50f);

                if(me->HasAura(SPELL_RAISE_ALLY_GLYPH))
                {
                    ghoul_speed += 15.0f;
                    ghoul_health += uint32(creator->GetMaxHealth() * 0.25f);
                }

                me->SetLevel(creator->getLevel());
                me->setFaction(creator->getFaction());
                me->SetSpeed(MOVE_RUN, ghoul_speed, true);
                me->SetMaxHealth(ghoul_health);
            }
        }
        ...
        void JustDied(Unit* killer)
        {
            ...
            if (Unit* owner = me->GetOwner())
                owner->RemoveAurasDueToSpell(SPELL_RAISE_ALLY_TRIGGERED);
        }
        ...
        void UpdateAI(const uint32 uiDiff)
        {
            ...
            Unit* owner = me->GetOwner();
            if(!me->isCharmed() || (owner && !owner->HasAura(SPELL_RAISE_ALLY_TRIGGERED)))
                me->ForcedDespawn();
