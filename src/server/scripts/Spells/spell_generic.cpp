...
class spell_gen_taunt_flag : public SpellScriptLoader
{
public:
    spell_gen_taunt_flag() : SpellScriptLoader("spell_gen_taunt_flag") { }

    class spell_gen_taunt_flag_SpellScript : public SpellScript
    {
        PrepareSpellScript(spell_gen_taunt_flag_SpellScript)
        bool Validate(SpellEntry const * /*spellEntry*/)
        {
            if (!sSpellStore.LookupEntry(SPELL_TAUNT_FLAG_SUMMON))
                return false;
            return true;
        }
        ...
        void HandleScriptEffect(SpellEffIndex /*effIndex*/)
        {
            if (Unit* caster = GetCaster())
            {
                ...
                {
                    if (unitTarget->ToPlayer() && unitTarget->IsHostileTo(caster) && !unitTarget->isAlive())
                    {
                        std::string pText;
                        pText += caster->GetName();
                        pText += " has placed the flag of ownership in ";
                        pText += unitTarget->GetName();
                        pText += "corpse.";
                        
                        unitTarget->CastSpell(unitTarget,SPELL_TAUNT_FLAG_SUMMON,true);
                        caster->ToPlayer()->TextEmote(pText);
                        return;
                    }
                }
        ...
        void Register()
        {
            OnEffectHitTarget += SpellEffectFn(spell_gen_taunt_flag_SpellScript::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_DUMMY);
        }
    };

    SpellScript* GetSpellScript() const
    {
        return new spell_gen_taunt_flag_SpellScript();
    }
};
