...
enum DeathKnightSpells
{
    ...
    DK_SPELL_RAISE_ALLY_GLYPH                   = 60200,
};
    ...
    class spell_dk_raise_ally_SpellScript : public SpellScript
    {
        ...
        void HandleScriptEffect(SpellEffIndex /*effIndex*/)
        {
            if (Unit* caster = GetCaster())
            {
                ...
                {
                    if (unitTarget->ToPlayer() && !unitTarget->isAlive() && (unitTarget->IsInPartyWith(caster) || unitTarget->IsInRaidWith(caster)))
                    {
                        unitTarget->CastSpell(unitTarget, DK_SPELL_RAISE_ALLY_TRIGGERED, true);
                        return;
                    }
                }
                ...
                caster->ToPlayer()->GetGlobalCooldownMgr().CancelGlobalCooldown(GetSpellInfo());
            }
        }

        void Register()
        {
            OnEffectHitTarget += SpellEffectFn(spell_dk_raise_ally_SpellScript::HandleScriptEffect, EFFECT_0, SPELL_EFFECT_DUMMY);
        }
    };
...
// 46619 Raise Ally Effect
class spell_dk_raise_ally_effect : public SpellScriptLoader
{
public:
    spell_dk_raise_ally_effect() : SpellScriptLoader("spell_dk_raise_ally_effect") { }

    class spell_dk_raise_ally_effect_AuraScript : public AuraScript
    {
        PrepareAuraScript(spell_dk_raise_ally_effect_AuraScript)
        bool Validate(SpellEntry const* /*entry*/)
        {
            if (!sSpellStore.LookupEntry(DK_SPELL_RAISE_ALLY_GLYPH))
                return false;
        ...
        void HandleEffectRemove(AuraEffect const * /*aurEff*/, AuraEffectHandleModes /*mode*/)
        {
            ...
            {
                if(Minion* ghoul = unitTarget->GetFirstMinion())
                    if (ghoul->isAlive())
                        unitTarget->Kill(ghoul);
            }
        }

        void Register()
        {
            OnEffectRemove += AuraEffectRemoveFn(spell_dk_raise_ally_effect_AuraScript::HandleEffectRemove, EFFECT_1, SPELL_AURA_DUMMY, AURA_EFFECT_HANDLE_REAL);
        }
    };

    AuraScript *GetAuraScript() const
    {
        return new spell_dk_raise_ally_effect_AuraScript();
    }
};
...
void AddSC_deathknight_spell_scripts()
{
    ...
    new spell_dk_raise_ally_effect();
