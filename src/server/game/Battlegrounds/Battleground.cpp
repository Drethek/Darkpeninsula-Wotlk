    ...
    switch (GetStatus())
    {
        ...
        case STATUS_IN_PROGRESS:
            ...
            else
            {
                ...
                UpdateBalance();
                if (sBattlegroundMgr->GetPrematureFinishTime() && (GetPlayersCountByTeam(ALLIANCE) < GetMinPlayersPerTeam() || GetPlayersCountByTeam(HORDE) < GetMinPlayersPerTeam() || HasBalance()))
...
void Battleground::EndBattleground(uint32 winner)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_BATTLEGROUND_MOD_ENABLE))
    {
        if(isBattleground() && !isArena())
        {
            uint32 Honor = 0;
            uint32 Loser = (winner == ALLIANCE) ? HORDE : ALLIANCE;
            switch (GetTypeID())
            {
                case BATTLEGROUND_AV: Honor += sWorld->getIntConfig(CONFIG_BATTLEGROUND_MOD_AV); break;
                case BATTLEGROUND_WS: Honor += sWorld->getIntConfig(CONFIG_BATTLEGROUND_MOD_WS); break;
                case BATTLEGROUND_AB: Honor += sWorld->getIntConfig(CONFIG_BATTLEGROUND_MOD_AB); break;
                case BATTLEGROUND_EY: Honor += sWorld->getIntConfig(CONFIG_BATTLEGROUND_MOD_EY); break;
                case BATTLEGROUND_SA: Honor += sWorld->getIntConfig(CONFIG_BATTLEGROUND_MOD_SA); break;
                case BATTLEGROUND_IC: Honor += sWorld->getIntConfig(CONFIG_BATTLEGROUND_MOD_IC); break;
                default: break;
            }
            
            RewardHonorToTeam(Honor, winner);
            RewardHonorToTeam(uint32(Honor/2), Loser);
        }
    }
