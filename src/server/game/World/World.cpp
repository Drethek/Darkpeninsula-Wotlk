...
void World::LoadConfigSettings(bool reload)
{
    ...
    m_bool_configs[CONFIG_ARENA_QUEUE_ANNOUNCER_ARENA_CHANNEL_ONLY]  = ConfigMgr::GetBoolDefault("Arena.QueueAnnouncer.PlayerArenaChannelOnly", false);
    ...
    m_int_configs[CONFIG_BALANCE_MINIMUM] = ConfigMgr::GetIntDefault("Battleground.Balance", 1800);

    // Custom Mods By Darkpeninsula
    m_bool_configs[CONFIG_BATTLEGROUND_MOD_ENABLE] = ConfigMgr::GetBoolDefault("Battleground.Mod.Enable", false);
    m_int_configs[CONFIG_BATTLEGROUND_MOD_AV] = ConfigMgr::GetIntDefault("Battleground.Mod.Alterac.Valley", 0);
    m_int_configs[CONFIG_BATTLEGROUND_MOD_WS] = ConfigMgr::GetIntDefault("Battleground.Mod.Warsong.Gulch", 0);
    m_int_configs[CONFIG_BATTLEGROUND_MOD_AB] = ConfigMgr::GetIntDefault("Battleground.Mod.Arathi.Basin", 0);
    m_int_configs[CONFIG_BATTLEGROUND_MOD_EY] = ConfigMgr::GetIntDefault("Battleground.Mod.Eye.Of.The.Storm", 0);
    m_int_configs[CONFIG_BATTLEGROUND_MOD_SA] = ConfigMgr::GetIntDefault("Battleground.Mod.Strand.Of.The.Ancients", 0);
    m_int_configs[CONFIG_BATTLEGROUND_MOD_IC] = ConfigMgr::GetIntDefault("Battleground.Mod.Isle.Of.Conquest", 0);
...
/// Send a System Message to all players (if player are in Arena Channel)
void World::SendArenaAnnounceText(int32 string_id, ...)
{
    va_list ap;
    va_start(ap, string_id);
    ...
    for (SessionMap::const_iterator itr = m_sessions.begin(); itr != m_sessions.end(); ++itr)
    {
        ...
        if(ChannelMgr* cMgr = channelMgr(itr->second->GetPlayer()->GetTeam()))
        {
            if(Channel *chn = cMgr->GetChannel("Arena", itr->second->GetPlayer()))
            {
                if(!chn->FindPlayer(itr->second->GetPlayer()->GetGUID()))
                    continue;
            }
            else
                continue;
