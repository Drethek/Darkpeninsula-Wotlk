    ...
    if (m_MirrorTimerFlags & (UNDERWATER_INLAVA|UNDERWATER_INSLIME))
    {
        ...
        else
        {
            ...
            if (m_MirrorTimer[FIRE_TIMER] < 0)
            {
                ...
                else if (m_zoneUpdateId != 1497 && m_mapId != 572)
...
void Player::SendNewItem(Item* item, uint32 count, bool received, bool created, bool broadcast)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_LOGS_ITEMS_ENABLE))
    {
        ItemTemplate const *pProto = sObjectMgr->GetItemTemplate(item->GetEntry());
		if(pProto)
        {
            if(pProto->ItemLevel < sWorld->getIntConfig(CONFIG_LOGS_ITEMS_LEVEL))
                return;

            if(pProto->Quality < sWorld->getIntConfig(CONFIG_LOGS_ITEMS_QUALITY))
                return;
            
            SQLTransaction trans = LogDatabase.BeginTransaction();
            PreparedStatement* stmt = LogDatabase.GetPreparedStatement(LOGS_ADD_ITEM);
            stmt->setUInt32(0, GetGUID());
            stmt->setUInt32(0, item->GetEntry());
            stmt->setUInt32(0, count);
            stmt->setUInt32(0, uint32(received));
            stmt->setUInt32(0, uint32(created));
            trans->Append(stmt);
        }
    }
}
...
#define RelocateToHomebind(){ mapId = m_homebindMapId; instanceId = 0; Relocate(m_homebindX, m_homebindY, m_homebindZ); }
    ...
    if(pvpWG)
    {
        // Removed All Player In Queue For Wintegrasp
        if(pvpWG->PlayerInWait[GetTeamId()].count(GetGUID()))
            pvpWG->PlayerInWait[GetTeamId()].erase(GetGUID());

        // Removed All Player In Battle For Wintegrasp
        if(pvpWG->PlayerInWar[GetTeamId()].count(GetGUID()))
            pvpWG->PlayerInWar[GetTeamId()].erase(GetGUID());

        sLog->outDebug(LOG_FILTER_BATTLEGROUND, "Removed All Wintergrasp States");
    }
