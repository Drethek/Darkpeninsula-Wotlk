...
bool Unit::HandleAuraRaidProcFromChargeWithValue(AuraEffect* triggeredByAura)
{
    ...
    if (this->HasAuraType(SPELL_AURA_SCHOOL_ABSORB) || !(spellProto->SpellFamilyName == SPELLFAMILY_PRIEST && spellProto->SpellFamilyFlags[1] & 0x20))
    {
...
void Unit::Kill(Unit* victim, bool durabilityLoss)
{
    ...
    if (sWorld->getBoolConfig(CONFIG_LOGS_KILL_ENABLE))
    {
        if (victim->GetTypeId() != TYPEID_UNIT)
            return;

        if(victim->getLevel() < sWorld->getIntConfig(CONFIG_LOGS_KILL_LEVEL))
            return;

        if(victim->ToCreature()->GetCreatureInfo()->rank == sWorld->getIntConfig(CONFIG_LOGS_KILL_RANK))
            return;
        
        SQLTransaction trans = LogDatabase.BeginTransaction();

        PreparedStatement* stmt = LogDatabase.GetPreparedStatement(LOGS_KILL_CREATURE);
        stmt->setUInt32(0, victim->ToCreature()->GetEntry());
        ...
        if (Group *group = player->GetGroup())
        {
            for (GroupReference *itr = group->GetFirstMember(); itr != NULL; itr = itr->next())
            {
                ...
                {
                    PreparedStatement* stmt2 = LogDatabase.GetPreparedStatement(LOGS_KILL_CREATURE_GROUP);
                    stmt2->setUInt32(0, group->GetGUID());
                    stmt2->setUInt32(0, member->GetGUID());
                    trans->Append(stmt2);
                }
            }
