...
AuctionHouseBot::AuctionHouseBot()
{
    ...
    DisableRecipeBelowLevel = 0;
    DisableRecipeAboveLevel = 0;
...
void AuctionHouseBot::Initialize()
{
    ...
    DisableRecipeBelowLevel = ConfigMgr::GetIntDefault("AuctionHouseBot.DisableRecipeBelowLevel", 0);
    DisableRecipeAboveLevel = ConfigMgr::GetIntDefault("AuctionHouseBot.DisableRecipeAboveLevel", 0);
    ...
    if (AHBSeller)
    {
        ...
        {
            ItemTemplate const* prototype = sObjectMgr->GetItemTemplate(itr->second.ItemId);
            ...
            // Disable Recipe below level X
            if ((DisableRecipeBelowLevel) && (prototype->Class == ITEM_CLASS_RECIPE) && (prototype->ItemLevel < DisableRecipeBelowLevel))
            {
                if (debug_Out_Filters) sLog->outString("AuctionHouseBot: Recipe %u disabled (Recipe Level = %u)", prototype->ItemId, prototype->ItemLevel);
                continue;
            }

            // Disable Recipe above level X
            if ((DisableRecipeAboveLevel) && (prototype->Class == ITEM_CLASS_RECIPE) && (prototype->ItemLevel > DisableRecipeAboveLevel))
            {
                if (debug_Out_Filters) sLog->outString("AuctionHouseBot: Recipe %u disabled (Recipe Level = %u)", prototype->ItemId, prototype->ItemLevel);
                continue;
            }
