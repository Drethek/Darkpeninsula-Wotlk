...
void WardenWin::HandleData(ByteBuffer &buff)
{
    ...
    for (std::vector<uint32>::iterator itr = SendDataId.begin(); itr != SendDataId.end(); ++itr)
    {
        ...
        switch (type)
        {
            case MEM_CHECK:
            {
                ...
                if (Mem_Result != 0)
                {
                    sLog->outWarden("RESULT MEM_CHECK not 0x00, CheckId %u account Id %u player GUID %u player name %s", *itr, Client->GetAccountId(), Client->GetGUIDLow(), Client->GetPlayerName());
                    found = true;
                    continue;
                }
                ...
                {
                    sLog->outWarden("RESULT MEM_CHECK fail, CheckId %u account Id %u player GUID %u player name %s", *itr, Client->GetAccountId(), Client->GetGUIDLow(), Client->GetPlayerName());
                    found = true;
                ...
                buff.rpos(buff.rpos() + rd->Length);
                sLog->outStaticDebug("RESULT MEM_CHECK passed, CheckId %u account Id %u player GUID %u player name %s", *itr, Client->GetAccountId(), Client->GetGUIDLow(), Client->GetPlayerName());
                break;
            }
            case PAGE_CHECK_A:
            case PAGE_CHECK_B:
            ...
            case MODULE_CHECK:
            {
                ...
                if (memcmp(buff.contents() + buff.rpos(), &byte, sizeof(uint8)) != 0)
                {
                    if (type == PAGE_CHECK_A || type == PAGE_CHECK_B)
                        sLog->outWarden("RESULT PAGE_CHECK fail, CheckId %u account Id %u player GUID %u player name %s", *itr, Client->GetAccountId(), Client->GetGUIDLow(), Client->GetPlayerName());
                    if (type == MODULE_CHECK)
                        sLog->outWarden("RESULT MODULE_CHECK fail, CheckId %u account Id %u player GUID %u player name %s", *itr, Client->GetAccountId(), Client->GetGUIDLow(), Client->GetPlayerName());
                    if (type == DRIVER_CHECK)
                        sLog->outWarden("RESULT DRIVER_CHECK fail, CheckId %u account Id %u player GUID %u player name %s", *itr, Client->GetAccountId(), Client->GetGUIDLow(), Client->GetPlayerName());
                    found = true;
                ...
                buff.rpos(buff.rpos() + 1);
                if (type == PAGE_CHECK_A || type == PAGE_CHECK_B)
                    sLog->outStaticDebug("RESULT PAGE_CHECK passed, CheckId %u account Id %u player GUID %u player name %s", *itr, Client->GetAccountId(), Client->GetGUIDLow(), Client->GetPlayerName());
                else if (type == MODULE_CHECK)
                    sLog->outStaticDebug("RESULT MODULE_CHECK passed, CheckId %u account Id %u player GUID %u player name %s", *itr, Client->GetAccountId(), Client->GetGUIDLow(), Client->GetPlayerName());
                else if (type == DRIVER_CHECK)
                    sLog->outStaticDebug("RESULT DRIVER_CHECK passed, CheckId %u account Id %u player GUID %u player name %s", *itr, Client->GetAccountId(), Client->GetGUIDLow(), Client->GetPlayerName());
                break;
            }
            case LUA_STR_CHECK:
            {
                ...
                if (Lua_Result != 0)
                {
                    sLog->outWarden("RESULT LUA_STR_CHECK fail, CheckId %u account Id %u player GUID %u player name %s", *itr, Client->GetAccountId(), Client->GetGUIDLow(), Client->GetPlayerName());
                    found = true;
                    continue;
                }
                ...
                sLog->outStaticDebug("RESULT LUA_STR_CHECK passed, CheckId %u account Id %u player GUID %u player name %s", *itr, Client->GetAccountId(), Client->GetGUIDLow(), Client->GetPlayerName());
                break;
            }
            case MPQ_CHECK:
            {
                ...
                if (Mpq_Result != 0)
                {
                    sLog->outWarden("RESULT MPQ_CHECK not 0x00 account Id %u player GUID %u player name %s", Client->GetAccountId(), Client->GetGUIDLow(), Client->GetPlayerName());
                    found = true;
                    continue;
                }
                ...
                {
                    sLog->outWarden("RESULT MPQ_CHECK fail, CheckId %u account Id %u player GUID %u player name %s", *itr, Client->GetAccountId(), Client->GetGUIDLow(), Client->GetPlayerName());
                    found = true;
                ...
                buff.rpos(buff.rpos() + 20);                // 20 bytes SHA1
                sLog->outStaticDebug("RESULT MPQ_CHECK passed, CheckId %u account Id %u player GUID %u player name %s", *itr, Client->GetAccountId(), Client->GetGUIDLow(), Client->GetPlayerName());
                break;
            }
    ...
    if (found && sWorld->getIntConfig(CONFIG_INT_WARDEN_BANDAY))
    {
        ...
        std::string sText = ("Hack Player: " + std::string(Client->GetPlayerName()) + " - Ban: " + sDuratuin.c_str());
    ...
    {
        std::string sText = ("Hack Player: " + std::string(Client->GetPlayerName()));
        sWorld->SendGMText(LANG_GM_BROADCAST, sText.c_str());
        Client->KickPlayer();
    }
    else if(found)
