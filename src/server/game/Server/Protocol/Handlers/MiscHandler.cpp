...
{
    // empty opcode
    sLog->outDebug(LOG_FILTER_BATTLEGROUND, "WORLD: Recvd CMSG_HEARTH_AND_RESURRECT");
 
    // Wintergrasp Check Opcode
    if(_player->GetZoneId() == ZONE_WINTERGRASP)
    {
        // Check if WG is Enable
        if( !(sWorld->getBoolConfig(CONFIG_OUTDOORPVP_WINTERGRASP_ENABLED)) )
            return;
 
        uint64 player_guid = GetPlayer()->GetGUID();
        Player *plr = ObjectAccessor::FindPlayer(player_guid);

        //Check is WG is Started
        if(sWorld->GetWintergrapsState() == 0)
        {
            OutdoorPvPWG *pvpWG = (OutdoorPvPWG*)sOutdoorPvPMgr->GetOutdoorPvPToZoneId(4197);
            pvpWG->HandlePlayerLeaveZone(plr,4197);
        }
