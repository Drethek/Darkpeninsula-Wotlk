...
namespace Trinity
{
    ...
    {
        public:
            RaiseAllyObjectCheck(Unit* funit, float range) : i_funit(funit), i_range(range) {}
            bool operator()(Player* u)
            {
                if( u->isAlive() || u->isInFlight() || !i_funit->IsFriendlyTo(u) || !i_funit->IsInPartyWith(u) || !i_funit->IsInRaidWith(u))
                    return false;
